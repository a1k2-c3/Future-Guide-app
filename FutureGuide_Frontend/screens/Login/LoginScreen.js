...
export default function LoginScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      Alert.alert('Success', 'Login successful! Welcome back!', [
        { text: 'OK', onPress: () => { handleling(data) } }
      ]);
    } catch (error) {
  ...
  const handleling = async (data) => {
    try {
      navigation.replace('Main');
      const object = { login_id: data.login_id, profile_id: data.profileId };
      setLoginId(object);
      // console.log(object);
      await Storage.setItemAsync("user", JSON.stringify(object));
    }
    catch (err) {
      console.log(err);
    }
